...
import {
  getDemandByDate,
  getDemandByCurrIdAndDate,
} from '../../redux/actions/demand-actions';
...
const Diagram: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (stringId === 'default') {
      ...
      getDemandByCurrIdAndDate(Number(stringId), startDate, endDate).then(
        (res) => setDemandData(res)
      );
    }
  }, [currCurriculum]);
  ...
  const renderPickerItems = () => {
    ...
    for (let i = 0; i < allCurricula.length; i++) {
      if (!filteredArr[allCurricula[i].curriculumname]) {
        filteredArr[allCurricula[i].curriculumname] =
          allCurricula[i].curriculumid;
      }
    }
    ...
    if (result < 0) {
      return (
        <View style={styles.badResultNumbers}>
          <Text style={styles.statOverflowText}>UNDER</Text>
          <Text style={styles.statOverflowText}>{result}</Text>
        </View>
      );
    } else if (result === 0) {
      return (
        <View style={styles.resultNumbers}>
          <Text style={styles.statOverflowText}>MEET</Text>
          <Text style={styles.statOverflowText}>{result}</Text>
        </View>
      );
    } else {
      return (
        <View style={styles.resultNumbers}>
          <Text style={styles.statOverflowText}>OVERFLOW</Text>
          <Text style={styles.statOverflowText}>{result}</Text>
        </View>
      );
    }
  ...
  return (
    <View style={screenStyles.mainView}>
      ...
      <View style={styles.infoContainer}>
        <View style={styles.curriculaNameContainer}>
          <Text
            style={styles.curriculaName}
          >{`${currCurriculum[0]} - YTD`}</Text>
        </View>
